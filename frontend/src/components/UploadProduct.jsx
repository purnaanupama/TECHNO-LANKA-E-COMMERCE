...
const UploadProduct = ({ onClose, fetchData }) => {
  ...
  const handleUploadProductData=async(e)=>{
    console.log(data);
    e.preventDefault();
    setProductUpload(true);
    const response = await fetch('http://localhost:8000/api/product/create-product',{
      method: 'POST',
      credentials:'include',
      headers: {
    ...
    if(responseData.success){
      ...
      onClose();
      fetchData();
       toast.success("product creation success", {
  ...
  return (
    <div className='fixed w-full h-full bg-[rgba(0,0,0,0.6)] top-0 p-[120px] right-0 bottom-0 flex justify-center items-center'>
      <div className='bg-white relative p-6 rounded-md w-full h-[565px] min-w-[700px]'>
        <h1 className='px-6 py-2 bg-[#d4d4d4] w-[300px] mb-[5px] text-lg'>Upload Product</h1>
        <IoIosCloseCircle onClick={onClose} className='absolute top-2 right-2 text-2xl cursor-pointer hover:text-[#383737]' />
        <form className='flex flex-row w-[100%] gap-4 h-full'>
          ...
          <div className='p-[10px] w-[50%] flex flex-col gap-2'>
            ...
            <div className='grid py-4 h-[200px] px-5 w-[100%] outline outline-[#b3b3b3] overflow-x-auto'>
            <div className=' relative flex items-center gap-2 group group w-[600px]'>
          ...
          <label htmlFor="description">Description :</label> 
            ...
            <button type='submit' disabled={productUpload} className='py-2 customShadow2 px-2 text-xs bg-[#363636] mt-4 text-white hover:bg-[#515151]' onClick={handleUploadProductData}>{productUpload? "Saving...":"Save Product"}</button>
          </div>
        </form>
      </div>
      {/*display image full*/}
